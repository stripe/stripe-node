...
describe('Stripe Module', function() {
  ...
  describe('parseThinEvent__experimental', (done) => {
    const secret = 'whsec_test_secret';
    ...
    it('should parse webhook with a functioning pull method', (done) => {
      ...
      return getTestServerStripe(
        ...
        (req, res) => {
          if (
            req.url === '/v2/core/events/evt_123' &&
            req.headers['stripe-account'] == null
          ) {
            res.write(JSON.stringify(jsonWithData));
          } else {
            res.writeHead(404);
        ...
        async (err, stripe, closeServer) => {
          ...
          try {
            ...
            const event = stripe.parseThinEvent__experimental(
              payload,
              header,
              secret
            );
    ...
    it('should use the context property when pulling, if available', (done) => {
      ...
      return getTestServerStripe(
        ...
        (req, res) => {
          if (
            req.url === '/v2/core/events/evt_123' &&
            req.headers['stripe-account'] === 'acct_123'
          ) {
            res.write(JSON.stringify(jsonWithData));
          } else {
            res.writeHead(404);
