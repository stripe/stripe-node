...
4. The `main.ts` has to follow some rules:
  ...
  1. Must have `#!/usr/bin/env npx ts-node` header to be directly runable.
  1. Must print `Webhook endpoint available at <url>` when the server is started.
  1. Must return `{"received":true}` and 200 status code for `charge.succeeded` event.
