...
function Stripe(key, version) {
  ...
  this.setLogLevel = function(logLevelToSet) {
    if (typeof logLevelToSet === 'undefined' || typeof logLevelToSet === 'null') {
      logLevel = null;
    } else if (LOG_LEVELS.indexOf(logLevelToSet) > -1) {
      logLevel = logLevelToSet;
    } else {
      throw new Error('Log Level should only be set to `debug`, `info` or null/undefined');
    }
  };

  if (process.env['STRIPE_LOG']) {
    this.setLogLevel(process.env['STRIPE_LOG']);
  }
