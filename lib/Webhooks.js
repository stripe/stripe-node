...
function validateComputedSignature(payload, header, details, expectedSignature, tolerance) {
    const signatureFound = !!details.signatures.filter(Webhook._stripe._platformFunctions.secureCompare.bind(Webhook._stripe._platformFunctions, expectedSignature)).length;
    if (!signatureFound) {
        // @ts-ignore
