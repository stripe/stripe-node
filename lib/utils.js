...
const utils = (module.exports = {
  isSerializableObject(obj) {
    ...
    try {
      ...
      return str === JSON.stringify(JSON.parse(str));
    } catch (_) {
      return false;
    }
  },
  ...
  isOptionsHash(o) {
    return (
      this.isSerializableObject(o) &&
      Object.keys(o).filter(
        (p) => OPTIONS_KEYS.has(p) && typeof o[p] === 'string'
      ).length > 0
    );
  },
  ...
  getDataFromArgs(args) {
    if (
      !Array.isArray(args) ||
      args.length < 1 ||
      !this.isSerializableObject(args[0])
    ) {
      return {};
    }
    ...
    const optionKeysInArgs = argKeys.filter((key) => OPTIONS_KEYS.has(key));
