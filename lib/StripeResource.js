...
StripeResource.prototype = {
  ...
  _errorHandler: function(req, callback) {
    ...
    return function(error, message) {
      ...
      callback.call(
        self,
        new Error.StripeConnectionError({
          message: message || 'An error occurred with our connection to Stripe',
          detail: error,
        }),
        null
      );
  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    if (requestProcessor) {
      requestProcessor(method, data, options.headers, function(err, buffer) {
        if (err) {
          ...
          return handleError(err, 'An error occurred while processing the file to Stripe.');
        }
        requestData = buffer;
        setHeadersAndMakeRequest();
      });
    ...
    function setHeadersAndMakeRequest() {
