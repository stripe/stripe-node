...
StripeResource.prototype = {
  ...
  _request: function(method, path, data, auth, callback) {
    ...
    function makeRequest() {
      ...
      var isSecureConnection = self._stripe.getApiField('protocol') == 'https';
      ...
      req.on('socket', function(socket) {
        socket.on((isSecureConnection ? 'secureConnect' : 'connect'), function() {
          if (isSecureConnection && BLACKLISTED_FINGERPRINTS.indexOf(socket.getPeerCertificate().fingerprint) > -1) {
            req.abort();
