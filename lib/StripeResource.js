...
StripeResource.prototype = {
  ...
  _addHeadersDirectlyToRawResponse(rawResponse, headers) {
    // For convenience, make some headers easily accessible on
    // lastResponse.

    // NOTE: Stripe responds with lowercase header names/keys.
    rawResponse.requestId = headers['request-id'];
    rawResponse.stripeAccount =
      rawResponse.stripeAccount || headers['stripe-account'];
    rawResponse.apiVersion =
      rawResponse.apiVersion || headers['stripe-version'];
    rawResponse.idempotencyKey =
      rawResponse.idempotencyKey || headers['idempotency-key'];
  },

  _makeResponseEvent(requestEvent, statusCode, headers) {
    const requestEndTime = Date.now();
    const requestDurationMs = requestEndTime - requestEvent.requestStartTime;
    ...
    return utils.removeNullish({
      ...
      request_start_time: requestEvent.requestStartTime,
  ...
  _streamingResponseHandler(requestEvent, callback) {
    return (res) => {
      ...
      this._addHeadersDirectlyToRawResponse(rawResponse, headers);

      // TODO - this needs to be registered somehow on the raw response...
      // res.once('end', () => {
    ...
    const makeRequest = (apiVersion, headers, numRetries) => {
      ...
      const req = this._stripe
        ...
        .makeRequest(
          ...
          this._stripe.getApiField('agent'),
          timeout
        );
      ...
      req
        .then((res) => {
          if (this._shouldRetry(res, requestRetries, maxRetries)) {
            ...
            if (options.streaming && res.getStatusCode() < 400) {
