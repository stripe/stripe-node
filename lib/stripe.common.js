...
function Stripe(key, config = {}) {
    ...
    this._api = {
        ...
        timeout: Stripe._utils.validateInteger('timeout', props.timeout, DEFAULT_TIMEOUT),
        maxNetworkRetries: Stripe._utils.validateInteger('maxNetworkRetries', props.maxNetworkRetries, 0),
        agent: agent,
    ...
    getUname(cb) {
        if (!Stripe._UNAME_CACHE) {
            Stripe._UNAME_CACHE = new Promise((resolve) => {
                Stripe._utils.safeExec('uname -a', (err, uname) => {
                    resolve(uname);
                });
            });
        }
