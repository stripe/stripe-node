...
class RequestSender {
    ...
    _makeResponseEvent(requestEvent, statusCode, headers) {
        ...
        return this._stripe._utils.removeNullish({
    ...
    _defaultIdempotencyKey(method, settings) {
        ...
        if (method === 'POST' && maxRetries > 0) {
            return `stripe-node-retry-${this._stripe._utils.uuid4()}`;
        }
        return null;
    }
    _makeHeaders(auth, contentLength, apiVersion, clientUserAgent, method, userSuppliedHeaders, userSuppliedSettings) {
        ...
        if (methodHasPayload || contentLength) {
            if (!methodHasPayload) {
                this._stripe._utils.emitWarning(`${method} method had non-zero contentLength but no payload is expected for this verb`);
            }
            defaultHeaders['Content-Length'] = contentLength;
        }
        return Object.assign(this._stripe._utils.removeNullish(defaultHeaders), 
        // If the user supplied, say 'idempotency-key', override instead of appending by ensuring caps are the same.
        this._stripe._utils.normalizeHeaders(userSuppliedHeaders));
    }
    _getUserAgentString() {
    ...
    _recordRequestMetrics(requestId, requestDurationMs) {
        if (this._stripe.getTelemetryEnabled() && requestId) {
            if (this._stripe._prevRequestMetrics.length > this._maxBufferedRequestMetric) {
                this._stripe._utils.emitWarning('Request metrics buffer is full, dropping telemetry message.');
            }
            else {
    ...
    _request(method, host, path, data, auth, options = {}, callback, requestDataProcessor = null) {
        ...
        const makeRequest = (apiVersion, headers, numRetries) => {
            ...
            // @ts-ignore
            const requestEvent = this._stripe._utils.removeNullish({
        ...
        if (requestDataProcessor) {
            ...
            prepareAndMakeRequest(null, this._stripe._utils.stringifyRequestData(data || {}));
        }
    }
}
