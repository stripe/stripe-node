...
class RequestSender {
        ...
        .then(
          (jsonResponse) => {
            if (jsonResponse.error) {
              ...
              if (statusCode === 401) {
                err = new Error_js_1.StripeAuthenticationError(
                  jsonResponse.error
                );
  ...
  static _shouldRetry(res, numRetries, maxRetries, error) {
    if (
      error &&
      numRetries === 0 &&
      HttpClient_js_1.HttpClient.CONNECTION_CLOSED_ERROR_CODES.includes(
        error.code
      )
    ) {
      return true;
  ...
  _request(
        ...
        .catch((error) => {
          if (
            ...
            const isTimeoutError =
              error.code &&
              error.code === HttpClient_js_1.HttpClient.TIMEOUT_ERROR_CODE;
            return callback(
              new Error_js_1.StripeConnectionError({
    ...
    if (requestDataProcessor) {
      ...
      prepareAndMakeRequest(
        null,
        (0, utils_js_1.stringifyRequestData)(data || {})
      );
    }
  }
}
