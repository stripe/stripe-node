...
stripe_core_js_1.StripeClient.initialize(new NodePlatformFunctions_js_1.NodePlatformFunctions());
...
const StripeConstructor = function (key, config) {
    ...
    if (!(this instanceof StripeConstructor)) {
        return new stripe_core_js_1.StripeClient(key, config);
    }
    // If called with `new`, return a new Stripe instance
    return new stripe_core_js_1.StripeClient(key, config);
};
// Copy all static properties from Stripe to the wrapper
Object.setPrototypeOf(StripeConstructor, stripe_core_js_1.StripeClient);
Object.setPrototypeOf(StripeConstructor.prototype, stripe_core_js_1.StripeClient.prototype);
// Copy static properties explicitly
for (const key of Object.getOwnPropertyNames(stripe_core_js_1.StripeClient)) {
    if (key !== 'length' && key !== 'prototype' && key !== 'name') {
        Object.defineProperty(StripeConstructor, key, {
            value: stripe_core_js_1.StripeClient[key],
            writable: true,
            enumerable: true,
            configurable: true,
        });
